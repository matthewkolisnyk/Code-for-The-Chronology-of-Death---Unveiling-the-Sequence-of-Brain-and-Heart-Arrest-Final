...
file_name = ""
Raw = mne.io.read_raw_edf(file_name,  preload=True, exclude = ['Event'])
...
'Oz': np.array([ 1.07600e-04, -1.14892e-01,  1.46570e-02]),
}
...
Raw.set_channel_types({"F3":"eeg","F4":"eeg","F7":"eeg","F8":"eeg","Fz":"eeg","P3":"eeg","P4":"eeg",\
                       "Pz":"eeg","C3":"eeg","C4":"eeg","O1":"eeg","O2":"eeg","Cz":"eeg","Fp1":"eeg","Fpz":"eeg","Fp2":"eeg","T3":"eeg",\
                           "T5":"eeg","T4":"eeg","T6":"eeg","A1":"eeg","A2":"eeg","X1":"ecg","X2":"ecg"})
Raw.set_montage(dig)    
...
# filter the data
eeg_raw = Raw.filter(l_freq = 0.1, h_freq = None, picks = ['eeg','eog'])
...
# next notch filter
eeg_raw = eeg_raw.notch_filter(freqs = np.arange(60,60+1),picks = ['all'])
...
# save as mat file for subsequent analysis.
...
import pandas as pd
ECG = pd.read_csv("", sep = "\t")['ECG2'].values
...
# resample ecg
ecg_raw = ecg_raw.resample(sfreq = 200)
