...
fs = 100;
WR = fs * 0.5;
[peakValues, peakIndices] = findpeaks(baselineBP,'MinPeakDistance',WR,'MinPeakHeight',mean(baselineBP));

% Find troughs by negating the signal
[troughValues, troughIndices] = findpeaks(-baselineBP,'MinPeakDistance',WR);
...
fs = 100;
WR = fs * 0.5;
[peakValues, peakIndices] = findpeaks(baselineV,'MinPeakDistance',WR,'MinPeakHeight',mean(baselineV));

% Find troughs by negating the signal
[troughValues, troughIndices] = findpeaks(-baselineV,'MinPeakDistance',WR);
troughValues = -troughValues; % Negate back to get the actual trough values
