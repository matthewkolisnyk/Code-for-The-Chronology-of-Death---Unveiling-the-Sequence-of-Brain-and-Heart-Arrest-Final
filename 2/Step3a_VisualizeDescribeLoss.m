%% get key information from TCD, ABP and EEG signals at the time of loss of brain blood flow
% note CCA = loss of brain blood and CA = loss of systemic circulation; ISO
...
CCATime = TCDClean.CCA_Matt + 28; % adjust slightly to line up with cardiac cycle
...
%% now for loss of systemic circulation
clear; close all;
...
%% finally for loss of brain activity 
clear; close all;
...
fs = 100;
WR = fs/1.1;
[peakValues, peakIndices] = findpeaks(BloodPressureISO,'MinPeakDistance',WR,'MinPeakHeight',mean(BloodPressureISO(1:buffer)));

% Find troughs by negating the signal
[troughValues, troughIndices] = findpeaks(-BloodPressureISO,'MinPeakDistance',WR);
troughValues = -troughValues; % Negate back to get the actual trough values
